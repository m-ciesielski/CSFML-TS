...
///%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//
		...
		if(camera_scroll==1)
		{
			...
			if(sfMouse_getPosition(window).x>=0 &&sfMouse_getPosition(window).x<MAP_SCROLL_THRESHOLD && sfView_getCenter(view).x-sfView_getSize(view).x/2+map_block_w>0)
			{
				camera_vect.x=-map_block_w/2;
				sfView_move(view,camera_vect);
			}
			else if(sfMouse_getPosition(window).x>sfWindow_getSize(window).x-MAP_SCROLL_THRESHOLD && sfView_getCenter(view).x<(map_w*map_block_w)-sfView_getSize(view).x/2+map_block_w )
			{
				camera_vect.x=map_block_w/2;
				sfView_move(view,camera_vect);
			}
			else if (sfMouse_getPosition(window).y>=0 && sfMouse_getPosition(window).y<MAP_SCROLL_THRESHOLD+top_bar_size.y && sfView_getCenter(view).y-sfView_getSize(view).y/2+map_block_h>0)
			{
				camera_vect.y=-map_block_h/2;
				sfView_move(view,camera_vect);
			}
...
///Mapping mouse position to main view coordinates
				...
				if(event.type==sfEvtKeyPressed && event.key.code==sfKeySpace)
				{
					...
					if(active_player==1)
						apply_command_morale_bonus(player1_army_size, player1_unit_list);
					if(active_player==2)
						apply_command_morale_bonus(player2_army_size, player2_unit_list);
					end_turn=1;
